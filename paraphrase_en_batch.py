...
def paraphrase(lines_list, translator_en_de, translator_de_en, sp_en_de, sp_de_en, gpu_id=-1, batch_size=1):
    ...
    trans_de_list = [tokenizer.tokenize(x, return_str=True).lower() for x in trans_de_list]
    trans_de_list = [' '.join(sp_de_en.EncodeAsPieces(x)) for x in trans_de_list]

    # print(trans_de_list)
    # trans_de = bpe_model.process_line(trans_de)

    data_de = BatchStringONMTDataset(trans_de_list, translator_de_en.fields)
    test_data_de = onmt.io.OrderedIterator(
        dataset=data_de, device=gpu_id, batch_size=len(trans_de_list), 
        train=False, sort=False, shuffle=False, sort_key=lambda x:-len(x.src))
    ...
    print(n_best_preds)
    # trans_en_list = [item[0] for item in n_best_preds]
    trans_en_list = [item for item in n_best_preds]

    print(trans_en_list)

    return trans_en_list
