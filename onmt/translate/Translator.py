    ...
    def translate_batch(self, batch, data):
        ...
        for i in range(self.max_length):
            ...
            for j, b in enumerate(beam):
                ...
                if not b.done():
                    log_prob[j,:] = log_prob[j,:] + torch.gather(out[:,j], 1, beam[j].get_current_state().view(-1,1)).view(-1)

            # log_prob += torch.gather(out.squeeze(1), 1, beam[0].get_current_state().view(-1,1)).view(-1)
