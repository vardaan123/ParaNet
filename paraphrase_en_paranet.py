...
def paraphrase(input_str, translator_en_de, translator_de_en, sp_en_de, sp_de_en, gpu_id=-1, batch_size=1):
    ...
    print(trans_de_k_best)

    # --------------------
    # German -> English
    # --------------------
    tokenizer = MosesTokenizer()
    trans_de_k_best = [tokenizer.tokenize(x, return_str=True) for x in trans_de_k_best]
    trans_de_k_best = [x.lower() for x in trans_de_k_best]
    trans_de_k_best = [' '.join(sp_de_en.EncodeAsPieces(x)) for x in trans_de_k_best]

    # trans_de_k_best = trans_de_k_best[0]
    # trans_de = bpe_model.process_line(trans_de)

    data_de = MultStringONMTDataset(trans_de_k_best, translator_de_en.fields)
    # data_de = StringONMTDataset(trans_de_k_best, translator_de_en.fields)

    test_data_de = onmt.io.OrderedIterator(
        dataset=data_de, device=gpu_id, batch_size=1, 
        train=False, sort=False, shuffle=False)
    ...
    batch_data = translator_de_en.translate_batch(prepared_strings, data_de, probs)
